...
plot.QA = function(x, ..., outDir = NULL){
  ...
  a = list(x,...)
  #pretty colors
  pyt <- c("#481567FF", "#33638DFF", "#20A387FF", "#95D840FF")
  ...
  if(n == 1){
    ...
    boxplot(pd, ylab = "Odds ratio (known origin:random)", 
            outline = FALSE)
    abline(1,0, col="#481567FF", lwd=2, lty=3)
    ...
    plot(c(0,1), c(1,0), type="l", col="#481567FF", lwd=2, lty=3,
         ...
         ylab="Proportion of area excluded", xlim=c(0,1), ylim=c(0,1))
    for(i in seq_len(n)){
      lines(pre[,1], pre[,i+1], lwd=2, col=pyt[i+1])
    }
    legend("bottomleft", nm, lwd=2, col=pyt[-1], bty="n")
    ...
    plot(c(0,1), c(0,1), type="l", col="#481567FF", lwd=2, lty=3,
         ...
         xlim=c(0,1), ylim=c(0,1))
    for(i in seq_len(n)){
      lines(means.p[,1], means.p[,i+1], lwd=2, col=pyt[i+1])
    }
    legend("bottomright", nm, lwd=2, col=pyt[-1], bty="n")
    ...
    plot(c(0,1), c(0,1), type="l", col="#481567FF", lwd=2, lty=3, 
         ...
         xlim=c(0,1), ylim=c(0,1))
    for(i in seq_len(n)){
      lines(means.a[,1], means.a[,i+1], lwd=2, col=pyt[i+1])
    }
    legend("bottomright", nm, lwd=2, col=pyt[-1], bty="n")  
    
    boxplot(pd, col=pyt, names = nm,
    ...
    if(!is.null(outDir)){
      ...
      plot(c(0,1), c(1,0), type="l", col="#481567FF", lwd=2, lty=3,
           ...
           ylim=c(0,1))
      for(i in seq_len(n)){
        lines(pre[,1], pre[,i+1], lwd=2, col="#481567FF"+1)
      }
      legend("bottomright", yp, nm, lwd=2, col=pyt, bty="n")
      text(0.95, 0.95, "(a)")
